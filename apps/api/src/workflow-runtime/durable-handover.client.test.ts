import type { HandoverDurableWorkflowInput } from '@careos/contracts';
...
import { DurableHandoverClient } from './durable-handover.client.js';

const handoverId = '44444444-4444-4444-8444-444444444444';
const instanceId = `handover-${handoverId}`;
const input: HandoverDurableWorkflowInput = {
  actor: {
...
describe('DurableHandoverClient', () => {
  afterEach(() => {
    vi.clearAllMocks();
    vi.unstubAllEnvs();
  });

  it('starts the versioned ID-only Handover orchestration', async () => {
    configure();
    ...
    await new DurableHandoverClient().start(instanceId, input);

    expect(clientMock.scheduleNewOrchestration).toHaveBeenCalledWith(
      'HandoverOrchestratorV1',
      input,
  ...
  it('reconciles another caller winning the start race', async () => {
    ...
    await expect(new DurableHandoverClient().start(instanceId, input)).resolves.toBeUndefined();
  });

  it('rejects reuse of a terminal Handover instance', async () => {
    configure();
    clientMock.getOrchestrationState.mockResolvedValue({
      runtimeStatus: OrchestrationStatus.COMPLETED,
    });

    await expect(new DurableHandoverClient().start(instanceId, input)).rejects.toThrow(/terminal/);
  });

  it('fails closed without scheduler configuration', async () => {
    vi.stubEnv('DURABLE_TASK_SCHEDULER_CONNECTION_STRING', '');

    await expect(new DurableHandoverClient().start(instanceId, input)).rejects.toThrow(
      /not configured/,
    );
  });
});
