import {
  ...
  type RetentionSweepDurableWorkflowInput,
...
export interface RetentionDurableClient {
  start(instanceId: string, input: RetentionSweepDurableWorkflowInput): Promise<void>;
}

@Injectable()
export class DurableRetentionClient implements RetentionDurableClient, OnModuleDestroy {
  private client: TaskHubGrpcClient | undefined;

  async start(instanceId: string, input: RetentionSweepDurableWorkflowInput): Promise<void> {
    ...
    if (existing !== undefined) {
      ...
      throw new ConflictException(`Durable Retention workflow ${instanceId} is already terminal.`);
    }
    try {
      await client.scheduleNewOrchestration(RETENTION_SWEEP_DURABLE_WORKFLOW_TYPE, input, {
        instanceId,
        version: RETENTION_DURABLE_VERSION,
      });
    } catch (startError) {
  ...
  private getClient(): TaskHubGrpcClient {
    ...
    if (connectionString === undefined || connectionString === '') {
      throw new ServiceUnavailableException(
        'Durable Task Scheduler is not configured for Retention workflows.',
      );
    }
